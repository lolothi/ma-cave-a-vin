      ...
      <div class="flex justify-between">
        <router-link to="/"
          ><div class="flex mr-10">
            <img src="../assets/wine_glasses_32.png" /><strong
              >Ma cave à Vin</strong
            >
          </div></router-link
        >
        <div class="hidden md:block" id="navbar-default">
          <ul class="flex flex-row">
            <li v-for="link in navBarLinks" :key="link.to">
              <router-link :class="styleLinks(link.to)" :to="link.to">{{
                link.label
              }}</router-link>
            </li>
          </ul>
...
let isMenuOpen = ref(isMobile.value? false : true);
